...
ROLE_PERMISSIONS = {
    'Administrator': ['dashboard', 'members', 'contributions', 'loans', 'repayments', 'distributions', 'meetings', 'reports', 'users', 'audit', 'exports'],
    'Chairperson': ['dashboard', 'loans', 'distributions', 'meetings', 'reports', 'audit', 'exports'],
    ...
    'Auditor': ['dashboard', 'reports', 'audit', 'exports'],
...
def loans():
    if request.method == 'POST':
        ...
        db.session.add(l); db.session.commit(); log_audit('ISSUE_LOAN', 'Loan', l.id, f'{l.member.full_name} borrowed {kwacha(l.principal)}; due {l.due_on}'); flash('Loan issued at 15% interest.'); return redirect(url_for('loans'))
...
def init_db():
    ...
    db.session.commit()
    db.session.add(AuditLog(user_id=admin.id, username='admin', full_name='System Administrator', role='Administrator', action='INIT_DB', entity='Database', details='Database initialized with sample members and default admin user'))
    db.session.commit()
